...
# save all_dataframes
with open('data/processed/all_dataframes.pkl', 'wb') as f:
    pickle.dump(all_dataframes, f)
    print(f"Saved all_dataframes dictionary with {len(all_dataframes)} entries to all_dataframes.pkl")

# save datasets
with open('data/processed/datasets_by_category.pkl', 'wb') as f:
    pickle.dump(datasets, f)
    print(f"Saved datasets dictionary to datasets_by_category.pkl")

# save df as individual csvs
for urn, df in all_dataframes.items():
    clean_urn = urn.replace(":", "_")
    output_path = os.path.join('data/processed', f"{clean_urn}.csv")
    df.to_csv(output_path, index=False)
    print(f"Saved {urn} to {output_path}")

# save dictionary mapping
with open('data/processed/urn_metadata.json', 'w') as f:
    import json
    # convert dicts to serializable format
    metadata = {
        'urn_to_description': urn_to_description,
        'urn_categories': urn_categories,
        'urn_partners': {k: str(v) for k, v in urn_partners.items()}, 
        'urn_blocks': {k: str(v) for k, v in urn_blocks.items()} 
    }
    json.dump(metadata, f, indent=2)
    print("saved metadata to data/processed/urn_metadata.json")